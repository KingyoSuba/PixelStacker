...
//    {
//        #region SKBitmap
//        public static bool AreEqual(this SKBitmap L, SKBitmap R)
//        {
//            if (L != null ^ R != null) return false;
//            if (L.Width != R.Width) return false;
//            if (R.Height != R.Height) return false;
//            if (L.ColorType != R.ColorType) return false;

//            //Get the SKBitmap data
//            var srcData = L.LockBits(new Rectangle(0, 0, L.Width, L.Height), ImageLockMode.ReadOnly, L.ColorType);
//            var dstData = R.LockBits(new Rectangle(0, 0, R.Width, R.Height), ImageLockMode.ReadOnly, R.ColorType);
...
//            //Find pixelsize
//            int _ = Image.GetColorTypeSize(L.ColorType); // bits per pixel
...
//        public static Color GetPixelSafely(this SKBitmap src, int x, int y)
//        {
//            if (src == null) return Color.Transparent;
//            if (x < 0 || y < 0 || x >= src.Width || y >= src.Height) return Color.Transparent;
//            if (!CanReadPixel(src.ColorType)) return Color.Transparent;
...
//        /// <returns></returns>
//        public static List<Color> GetColorsInImage(this SKBitmap src, int rgbBucketSize = 1)
//        {
...
//        /// <param name="normalize">If colors should be put into their buckets of 5 or 15 or or whatever.</param>
//        /// <returns></returns>
//        public static Color GetAverageColor(this SKBitmap src, int rgbBucketSize = 1)
//        {
//            long r = 0;
//            long g = 0;
//            long b = 0;
...
//        private static bool CanReadPixel(ColorType format)
//        {
//            switch (format)
//            {
//                case SKColorType.Format1bppIndexed:
//                case SKColorType.Format4bppIndexed:
//                case SKColorType.Format8bppIndexed:
//                    return true;
//                case SKColorType.Format32bppArgb:
//                    return true;
...
//        /// <param name="src"></param>
//        /// <returns></returns>
//        public static SKBitmap To32bppBitmap(this SKImage src, int width, int height)
//        {
//            lock (src)
//            {
//                SKBitmap output = new SKBitmap(width, height, SKColorType.Format32bppArgb);
...
//        /// <param name="src"></param>
//        /// <returns></returns>
//        public static SKBitmap To32bppSKBitmap(this Image src)
//        {
//            lock (src)
//            {
//                SKBitmap output = new SKBitmap(src.Width, src.Height, SKColorType.Format32bppArgb);
//                using (Graphics g = Graphics.FromImage(output))
//                {
//                    g.DrawImage(src, new Rectangle(0, 0, output.Width, output.Height));
//                    return output;
...
//        public static SKBitmap BlurBidirectional(this SKBitmap src, int radiusH, int radiusV)
//        {
//            SKBitmap output = new SKBitmap(src.Width, src.Height, SKColorType.Format32bppArgb);

//            using (var src32 = src.To32bppSKBitmap())
//            {
...
//        /// <returns></returns>
//        public static void ToCopyStream(this SKBitmap origImage, SKBitmap dstImage, CancellationToken? worker, Func<int, int, Color, Color> callback)
//        {
...
//            //Get the SKBitmap data
//            var SKBitmapData = origImage.LockBits(
...
//            //Initialize an array for all the image data
//            byte[] imageBytes = new byte[SKBitmapData.Stride * origImage.Height];

//            //Copy the SKBitmap data to the local array
//            System.Runtime.InteropServices.Marshal.Copy(SKBitmapData.Scan0, imageBytes, 0, imageBytes.Length);

//            //Unlock the SKBitmap
//            origImage.UnlockBits(SKBitmapData);

//            //Find pixelsize
//            int pixelSize = Image.GetColorTypeSize(origImage.ColorType); // bits per pixel
...
//            //Get the SKBitmap data
//            SKBitmapData = dstImage.LockBits(
//                new Rectangle(0, 0, dstImage.Width, dstImage.Height),
//                ImageLockMode.WriteOnly,
//                dstImage.ColorType
//            );

//            //Copy the changed data into the SKBitmap again.
//            System.Runtime.InteropServices.Marshal.Copy(imageBytes, 0, SKBitmapData.Scan0, imageBytes.Length);

//            //Unlock the SKBitmap
//            dstImage.UnlockBits(SKBitmapData);
//        }
...
//        /// <param name="origImage"></param>
//        /// <returns></returns>
//        public static void ToMergeStreamParallel(this SKBitmap srcImage, SKBitmap dstImage, CancellationToken? worker, Func<int, int, Color, Color, Color> callback)
//        {
//            if (srcImage.ColorType != SKColorType.Format32bppArgb)
//            {
//                throw new ArgumentException("src ColorType MUST be SKColorType.Format32bppArgb.");
//            }

//            if (dstImage.ColorType != SKColorType.Format32bppArgb)
//            {
//                throw new ArgumentException("dst ColorType MUST be SKColorType.Format32bppArgb.");
//            }
...
//            //Get the SKBitmap data
//            var srcData = srcImage.LockBits(new Rectangle(0, 0, srcImage.Width, srcImage.Height), ImageLockMode.ReadOnly, srcImage.ColorType);
//            var dstData = dstImage.LockBits(new Rectangle(0, 0, dstImage.Width, dstImage.Height), ImageLockMode.ReadOnly, dstImage.ColorType);
...
//            int numYProcessed = 0;
//            int bitsPerPixel = Image.GetColorTypeSize(srcImage.ColorType); // bits per pixel
...
//        /// <param name="origImage"></param>
//        /// <returns></returns>
//        public static SKBitmap ToMergeStream(this SKBitmap origImage, SKBitmap dstImage, CancellationToken? worker, Func<int, int, Color, Color, Color> callback)
//        {
...
//            SKBitmapData srcData;
//            SKBitmapData dstData;
//            byte[] srcImageBytes;
//            byte[] dstImageBytes;

//            byte[] outImageBytes;
//            var outImage = new SKBitmap(origImage.Width, origImage.Height, SKColorType.Format32bppArgb);
...
//            //    {
//                    //Get the SKBitmap data
//                    srcData = origImage.LockBits(new Rectangle(0, 0, origImage.Width, origImage.Height), ImageLockMode.ReadOnly, origImage.ColorType);
//                    dstData = dstImage.LockBits(new Rectangle(0, 0, dstImage.Width, dstImage.Height), ImageLockMode.ReadOnly, dstImage.ColorType);
//                    //Initialize an array for all the image data
...
//            //Get the SKBitmap data
//            SKBitmapData outData = outImage.LockBits(
...
//        /// <param name="origImage"></param>
//        /// <returns></returns>
//        public static void ToEditStream(this SKBitmap origImage, CancellationToken? worker, Func<int, int, Color, Color> callback)
//        {
//            if (origImage.ColorType != SKColorType.Format32bppArgb)
//            {
//                throw new ArgumentException("ColorType MUST be SKColorType.Format32bppArgb.");
...
//            //Get the SKBitmap data
//            SKBitmapData = origImage.LockBits(
//                new Rectangle(0, 0, origImage.Width, origImage.Height),
//                ImageLockMode.WriteOnly,
//                origImage.ColorType
//            );
...
//        /// <param name="origImage"></param>
//        /// <returns></returns>
//        public static void ToEditStreamParallel(this SKBitmap origImage, CancellationToken? worker, Func<int, int, Color, Color> callback)
//        {
//            if (origImage.ColorType != SKColorType.Format32bppArgb)
//            {
//                throw new ArgumentException("ColorType MUST be SKColorType.Format32bppArgb.");
...
//        /// <param name="origImage"></param>
//        /// <returns></returns>
//        public static void ToViewStream(this SKBitmap origImage, CancellationToken? worker, Action<int, int, Color> callback)
//        {
//            if (origImage.ColorType != SKColorType.Format32bppArgb)
//            {
//                throw new ArgumentException("ColorType MUST be SKColorType.Format32bppArgb.");
...
//        /// <param name="origImage"></param>
//        /// <returns></returns>
//        public static void ToViewStreamParallel(this SKBitmap origImage, CancellationToken? worker, Action<int, int, Color> callback)
//        {
//            if (origImage.ColorType != SKColorType.Format32bppArgb)
//            {
...
//            if (!image.SKColorType.IsIndexed())
//            {
//                string message = string.Format("Cannot retrieve a color count from a non-indexed image with pixel format '{0}'.", image.ColorType);
...
//            if (!image.SKColorType.IsIndexed())
//            {
//                string message = string.Format("Cannot store a palette to a non-indexed image with pixel format '{0}'.", image.ColorType);
//                throw new InvalidOperationException(message);
//            }
